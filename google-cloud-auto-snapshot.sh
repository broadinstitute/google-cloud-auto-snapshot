...
gcloud compute disks list --format='value(name,zone)'|grep $1| while read DISK_NAME ZONE; do
...
#
gcloud compute snapshots list --filter="creationTimestamp<$(date -v "-60d" "+%Y-%m-%d")" --regexp "(autogcs.*)" --uri | while read SNAPSHOT_URI; do
